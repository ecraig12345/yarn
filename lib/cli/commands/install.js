...
let run = exports.run = (() => {
  var _ref31 = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* (config, reporter, flags, args) {
    ...
    if (flags.lockfile === false) {
      lockfile = new (_lockfile2 || _load_lockfile2()).default();
    } else {
      lockfile = yield (_lockfile2 || _load_lockfile2()).default.fromDirectory(config.lockfileFolder, reporter);
    }
...
function _load_objectPath() {
  return _objectPath = _interopRequireDefault(require('object-path'));
}
...
function _load_lockfile2() {
  return _lockfile2 = _interopRequireDefault(require('../../lockfile'));
}
...
class Install {
  ...
  compareOriginalVsResolvedLockfile(workspaceLayout) {
    var _this3 = this;

    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      const resolvedPatterns = _this3.getFilteredResolvedPatterns(workspaceLayout);
      const resolvedLockfile = new (_lockfile2 || _load_lockfile2()).default().getLockfile(resolvedPatterns);

      const originalLockfile = (yield (_lockfile2 || _load_lockfile2()).default.fromDirectory(_this3.config.lockfileFolder, _this3.reporter)).cache;
      // the resolved lock file has "imploded" entries, need to do the same for the original
      Object.keys(originalLockfile).forEach(function (key) {
        return originalLockfile[key] = (0, (_lockfile || _load_lockfile()).implodeEntry)(key, originalLockfile[key]);
      });
  ...
  bailout(patterns, workspaceLayout) {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      ...
      if (_this4.flags.audit) {
        return false;
      }
      // PNP is so fast that the integrity check isn't pertinent
      if (_this4.config.plugnplayEnabled) {
        return false;
      }
      if (_this4.flags.skipIntegrityCheck || _this4.flags.force) {
        return false;
      }
      const lockfileCache = _this4.lockfile.cache;
      if (!lockfileCache) {
        return false;
      }
      const lockfileClean = _this4.lockfile.parseResultType === 'success';
      const match = yield _this4.integrityChecker.check(patterns, lockfileCache, _this4.flags, workspaceLayout);

      if (_this4.flags.frozenLockfile && (!lockfileClean || match.missingPatterns.length > 0 || !(yield _this4.compareOriginalVsResolvedLockfile(workspaceLayout)))) {
        throw new (_errors || _load_errors()).MessageError(_this4.reporter.lang('frozenLockfileError'));
      }

      const haveLockfile = yield (_fs || _load_fs()).exists(path.join(_this4.config.lockfileFolder, (_constants || _load_constants()).LOCKFILE_FILENAME));

      const lockfileIntegrityPresent = !_this4.lockfile.hasEntriesExistWithoutIntegrity();
      const integrityBailout = lockfileIntegrityPresent || !_this4.config.autoAddIntegrity;

      if (match.integrityMatches && haveLockfile && lockfileClean && integrityBailout) {
        _this4.reporter.success(_this4.reporter.lang('upToDate'));
        return true;
      }

      if (match.integrityFileMissing && haveLockfile) {
        // Integrity file missing, force script installations
        _this4.scripts.setForce(true);
        return false;
      }
      ...
      if (!patterns.length && !match.integrityFileMissing) {
        _this4.reporter.success(_this4.reporter.lang('nothingToInstall'));
        yield _this4.createEmptyManifestFolders();
        yield _this4.saveLockfileAndIntegrity(patterns, workspaceLayout);
        return true;
      }
  ...
  createEmptyManifestFolders() {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      if (_this5.config.modulesFolder) {
        // already created
        return;
      }

      for (var _iterator9 = _this5.rootManifestRegistries, _isArray9 = Array.isArray(_iterator9), _i9 = 0, _iterator9 = _isArray9 ? _iterator9 : _iterator9[Symbol.iterator]();;) {
        var _ref10;
        ...
        const folder = _this5.config.registries[registryName].folder;

        yield (_fs || _load_fs()).mkdirp(path.join(_this5.config.lockfileFolder, folder));
      }
    })();
  }
  ...
  getFlattenedDeps() {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      var _ref12 = yield _this6.fetchRequestFromCwd();

      const depRequests = _ref12.requests,
            rawPatterns = _ref12.patterns;


      yield _this6.resolver.init(depRequests, {});
      ...
      return _this6.flatten(rawPatterns);
    })();
  }
  ...
  init() {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      ...
      if (yield (_fs || _load_fs()).exists(path.join(_this7.config.lockfileFolder, (_constants || _load_constants()).NPM_SHRINKWRAP_FILENAME))) {
        _this7.reporter.warn(_this7.reporter.lang('shrinkwrapWarning'));
      }

      // warn if we have an npm lockfile
      if (yield (_fs || _load_fs()).exists(path.join(_this7.config.lockfileFolder, (_constants || _load_constants()).NPM_LOCK_FILENAME))) {
        _this7.reporter.warn(_this7.reporter.lang('npmLockfileWarning'));
      }

      if (_this7.config.plugnplayEnabled) {
        _this7.reporter.info(_this7.reporter.lang('plugnplaySuggestV2L1'));
        _this7.reporter.info(_this7.reporter.lang('plugnplaySuggestV2L2'));
      }

      let flattenedTopLevelPatterns = [];
      const steps = [];

      var _ref13 = yield _this7.fetchRequestFromCwd();
      ...
      const artifacts = yield _this7.integrityChecker.getArtifacts();
      if (artifacts) {
        _this7.linker.setArtifacts(artifacts);
        _this7.scripts.setArtifacts(artifacts);
      }

      if ((_packageCompatibility || _load_packageCompatibility()).shouldCheck(manifest, _this7.flags)) {
        steps.push((() => {
          var _ref14 = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* (curr, total) {
            _this7.reporter.step(curr, total, _this7.reporter.lang('checkingManifest'), emoji.get('mag'));
            yield _this7.checkCompatibility();
          });
      ...
      const audit = new (_audit || _load_audit()).default(_this7.config, _this7.reporter, { groups: (_constants || _load_constants()).OWNED_DEPENDENCY_TYPES });
      ...
      steps.push(function (curr, total) {
        return (0, (_hooks || _load_hooks()).callThroughHook)('resolveStep', (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
          _this7.reporter.step(curr, total, _this7.reporter.lang('resolvingPackages'), emoji.get('mag'));
          yield _this7.resolver.init(_this7.prepareRequests(depRequests), {
            isFlat: _this7.flags.flat,
            isFrozen: _this7.flags.frozenLockfile,
            workspaceLayout
          });
          topLevelPatterns = _this7.preparePatterns(rawPatterns);
          flattenedTopLevelPatterns = yield _this7.flatten(topLevelPatterns);
          return { bailout: !_this7.flags.audit && (yield _this7.bailout(topLevelPatterns, workspaceLayout)) };
        }));
      });

      if (_this7.flags.audit) {
        steps.push(function (curr, total) {
          return (0, (_hooks || _load_hooks()).callThroughHook)('auditStep', (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
            _this7.reporter.step(curr, total, _this7.reporter.lang('auditRunning'), emoji.get('mag'));
            if (_this7.flags.offline) {
              _this7.reporter.warn(_this7.reporter.lang('auditOffline'));
              return { bailout: false };
            }
            const preparedManifests = yield _this7.prepareManifests();
            ...
            const auditVulnerabilityCounts = yield audit.performAudit(mergedManifest, _this7.lockfile, _this7.resolver, _this7.linker, topLevelPatterns);
            ...
            return { bailout: yield _this7.bailout(topLevelPatterns, workspaceLayout) };
          }));
        });
      }
      ...
      steps.push(function (curr, total) {
        return (0, (_hooks || _load_hooks()).callThroughHook)('fetchStep', (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
          _this7.markIgnored(ignorePatterns);
          _this7.reporter.step(curr, total, _this7.reporter.lang('fetchingPackages'), emoji.get('truck'));
          const manifests = yield (_packageFetcher || _load_packageFetcher()).fetch(_this7.resolver.getManifests(), _this7.config);
          _this7.resolver.updateManifests(manifests);
          yield (_packageCompatibility || _load_packageCompatibility()).check(_this7.resolver.getManifests(), _this7.config, _this7.flags.ignoreEngines);
        }));
      });
      ...
      steps.push(function (curr, total) {
        return (0, (_hooks || _load_hooks()).callThroughHook)('linkStep', (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
          ...
          yield _this7.integrityChecker.removeIntegrityFile();
          _this7.reporter.step(curr, total, _this7.reporter.lang('linkingDependencies'), emoji.get('link'));
          flattenedTopLevelPatterns = _this7.preparePatternsForLinking(flattenedTopLevelPatterns, manifest, _this7.config.lockfileFolder === _this7.config.cwd);
          yield _this7.linker.init(flattenedTopLevelPatterns, workspaceLayout, {
            linkDuplicates: _this7.flags.linkDuplicates,
            ignoreOptional: _this7.flags.ignoreOptional
          });
        }));
      });
      ...
      if (_this7.config.plugnplayEnabled) {
        steps.push(function (curr, total) {
          return (0, (_hooks || _load_hooks()).callThroughHook)('pnpStep', (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
            const pnpPath = `${_this7.config.lockfileFolder}/${(_constants || _load_constants()).PNP_FILENAME}`;

            const code = yield (0, (_generatePnpMap || _load_generatePnpMap()).generatePnpMap)(_this7.config, flattenedTopLevelPatterns, {
              resolver: _this7.resolver,
              reporter: _this7.reporter,
              targetPath: pnpPath,
              workspaceLayout
            });
      ...
      steps.push(function (curr, total) {
        return (0, (_hooks || _load_hooks()).callThroughHook)('buildStep', (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
          _this7.reporter.step(curr, total, _this7.flags.force ? _this7.reporter.lang('rebuildingPackages') : _this7.reporter.lang('buildingFreshPackages'), emoji.get('hammer'));

          if (_this7.config.ignoreScripts) {
            _this7.reporter.warn(_this7.reporter.lang('ignoredScripts'));
          } else {
            yield _this7.scripts.init(flattenedTopLevelPatterns);
          }
        }));
      });
      ...
      if (_this7.flags.har) {
        steps.push((() => {
          var _ref21 = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* (curr, total) {
            ...
            const filename = path.join(_this7.config.cwd, `yarn-install_${formattedDate}.har`);
            _this7.reporter.step(curr, total, _this7.reporter.lang('savingHar', filename), emoji.get('black_circle_for_record'));
            yield _this7.config.requestManager.saveHar(filename);
          });
      ...
      if (yield _this7.shouldClean()) {
        steps.push((() => {
          var _ref22 = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* (curr, total) {
            _this7.reporter.step(curr, total, _this7.reporter.lang('cleaningModules'), emoji.get('recycle'));
            yield (0, (_autoclean || _load_autoclean()).clean)(_this7.config, _this7.reporter);
          });
      ...
      for (var _iterator11 = steps, _isArray11 = Array.isArray(_iterator11), _i11 = 0, _iterator11 = _isArray11 ? _iterator11 : _iterator11[Symbol.iterator]();;) {
        ...
        if (stepResult && stepResult.bailout) {
          ...
          if (auditFoundProblems) {
            _this7.reporter.warn(_this7.reporter.lang('auditRunAuditForDetails'));
          }
          _this7.maybeOutputUpdate();
      ...
      yield _this7.saveLockfileAndIntegrity(topLevelPatterns, workspaceLayout);
      yield _this7.persistChanges();
      _this7.maybeOutputUpdate();
      _this7.config.requestManager.clearCache();
      return flattenedTopLevelPatterns;
    })();
  }
  ...
  checkCompatibility() {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      var _ref24 = yield _this8.fetchRequestFromCwd();

      const manifest = _ref24.manifest;

      yield (_packageCompatibility || _load_packageCompatibility()).checkOne(manifest, _this8.config, _this8.flags.ignoreEngines);
    })();
  }
  ...
  persistChanges() {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      ...
      if (yield _this9.applyChanges(manifests)) {
  ...
  flatten(patterns) {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      if (!_this10.flags.flat) {
        return patterns;
      }

      const flattenedPatterns = [];

      for (var _iterator12 = _this10.resolver.getAllDependencyNamesByLevelOrder(patterns), _isArray12 = Array.isArray(_iterator12), _i12 = 0, _iterator12 = _isArray12 ? _iterator12 : _iterator12[Symbol.iterator]();;) {
        var _ref25;
        ...
        const infos = _this10.resolver.getAllInfoForPackageName(name).filter(function (manifest) {
          const ref = manifest._reference;
          invariant(ref, 'expected package reference');
          return !ref.ignore;
        });
        ...
        if (infos.length === 1) {
          ...
          flattenedPatterns.push(_this10.resolver.patternsByPackage[name][0]);
          continue;
        }
        ...
        const options = infos.map(function (info) {
          ...
          return {
            ...
            name: _this10.reporter.lang('manualVersionResolutionOption', ref.patterns.join(', '), info.version),
        ...
        const resolutionVersion = _this10.resolutions[name];
        if (resolutionVersion && versions.indexOf(resolutionVersion) >= 0) {
          // use json `resolution` version
          version = resolutionVersion;
        } else {
          version = yield _this10.reporter.select(_this10.reporter.lang('manualVersionResolution', name), _this10.reporter.lang('answer'), options);
          _this10.resolutions[name] = version;
        }

        flattenedPatterns.push(_this10.resolver.collapseAllVersionsOfPackage(name, version));
      }

      // save resolutions to their appropriate root manifest
      if (Object.keys(_this10.resolutions).length) {
        const manifests = yield _this10.config.getRootManifests();

        for (const name in _this10.resolutions) {
          const version = _this10.resolutions[name];

          const patterns = _this10.resolver.patternsByPackage[name];
          if (!patterns) {
            continue;
          }
          ...
          for (var _iterator13 = patterns, _isArray13 = Array.isArray(_iterator13), _i13 = 0, _iterator13 = _isArray13 ? _iterator13 : _iterator13[Symbol.iterator]();;) {
            ...
            manifest = _this10.resolver.getResolvedPattern(pattern);
            if (manifest) {
              break;
            }
          }
        ...
        yield _this10.config.saveRootManifests(manifests);
      }
  ...
  pruneOfflineMirror(lockfile) {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      const mirror = _this11.config.getOfflineMirrorPath();
      if (!mirror) {
        return;
      }
  ...
  saveLockfileAndIntegrity(patterns, workspaceLayout) {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      const resolvedPatterns = _this12.getFilteredResolvedPatterns(workspaceLayout);
      ...
      const lockfileBasedOnResolver = _this12.lockfile.getLockfile(resolvedPatterns);

      if (_this12.config.pruneOfflineMirror) {
        yield _this12.pruneOfflineMirror(lockfileBasedOnResolver);
      }

      // write integrity hash
      if (!_this12.config.plugnplayEnabled) {
        yield _this12.integrityChecker.save(patterns, lockfileBasedOnResolver, _this12.flags, workspaceLayout, _this12.scripts.getArtifacts());
      }

      // --no-lockfile or --pure-lockfile or --frozen-lockfile
      if (_this12.flags.lockfile === false || _this12.flags.pureLockfile || _this12.flags.frozenLockfile) {
        return;
      }

      const lockFileHasAllPatterns = patterns.every(function (p) {
        return _this12.lockfile.getLocked(p);
      });
      const lockfilePatternsMatch = Object.keys(_this12.lockfile.cache || {}).every(function (p) {
      ...
      const resolverPatternsAreSameAsInLockfile = Object.keys(lockfileBasedOnResolver).every(function (pattern) {
        const manifest = _this12.lockfile.getLocked(pattern);
      ...
      if (!_this12.flags.force && _this12.lockfile.parseResultType === 'success' && lockFileHasAllPatterns && lockfilePatternsMatch && resolverPatternsAreSameAsInLockfile && integrityPatternsAreSameAsInLockfile && patterns.length) {
        return;
      }

      // build lockfile location
      const loc = path.join(_this12.config.lockfileFolder, (_constants || _load_constants()).LOCKFILE_FILENAME);

      // write lockfile
      const lockSource = (0, (_lockfile || _load_lockfile()).stringify)(lockfileBasedOnResolver, false, _this12.config.enableLockfileVersions);
      yield (_fs || _load_fs()).writeFilePreservingEol(loc, lockSource);

      _this12._logSuccessSaveLockfile();
    })();
  }
  ...
  hydrate(ignoreUnusedPatterns) {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      const request = yield _this13.fetchRequestFromCwd([], ignoreUnusedPatterns);
      ...
      yield _this13.resolver.init(depRequests, {
        isFlat: _this13.flags.flat,
        isFrozen: _this13.flags.frozenLockfile,
        workspaceLayout
      });
      yield _this13.flatten(rawPatterns);
      _this13.markIgnored(ignorePatterns);

      // fetch packages, should hit cache most of the time
      const manifests = yield (_packageFetcher || _load_packageFetcher()).fetch(_this13.resolver.getManifests(), _this13.config);
      _this13.resolver.updateManifests(manifests);
      yield (_packageCompatibility || _load_packageCompatibility()).check(_this13.resolver.getManifests(), _this13.config, _this13.flags.ignoreEngines);

      // expand minimal manifests
      for (var _iterator15 = _this13.resolver.getManifests(), _isArray15 = Array.isArray(_iterator15), _i15 = 0, _iterator15 = _isArray15 ? _iterator15 : _iterator15[Symbol.iterator]();;) {
        var _ref28;
        ...
        if (type === 'link') {
          ...
          loc = _this13.config.generateModuleCachePath(ref);
        }
        const newPkg = yield _this13.config.readManifest(loc);
        yield _this13.resolver.updateManifest(ref, newPkg);
      }
  ...
  _checkUpdate() {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      let latestVersion = yield _this14.config.requestManager.request({
      ...
      _this14.config.registries.yarn.saveHomeConfig({
      ...
      if (semver.gt(latestVersion, (_yarnVersion || _load_yarnVersion()).version)) {
        ...
        _this14.maybeOutputUpdate = function () {
          _this14.reporter.warn(_this14.reporter.lang('yarnOutdated', latestVersion, (_yarnVersion || _load_yarnVersion()).version));

          const command = getUpdateCommand(installationMethod);
          if (command) {
            _this14.reporter.info(_this14.reporter.lang('yarnOutdatedCommand'));
            _this14.reporter.command(command);
          } else {
            const installer = getUpdateInstaller(installationMethod);
            if (installer) {
              _this14.reporter.info(_this14.reporter.lang('yarnOutdatedInstaller', installer));
            }
          }
        };
      }
