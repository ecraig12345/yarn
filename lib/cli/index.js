...
let main = exports.main = (() => {
  var _ref = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* ({
    ...
    (_commander || _load_commander()).default.option('--ignore-cpu', 'ignore CPU check');
    (_commander || _load_commander()).default.option('--ignore-platform', 'ignore platform ("os" and "cpu") checks');
    ...
    (_commander || _load_commander()).default.option('--error-log-file <path>', 'specify a custom file path to log error details to when an error occurs');
    ...
    (_commander || _load_commander()).default.option('--verbose-scripts', 'log verbose info and timing for scripts');
    ...
    function writeErrorReport(log) {
      const errorReportLoc = config.errorLogFile ? (_path || _load_path()).default.resolve(config.cwd, config.errorLogFile) // Use resolve to allow for relative and absolute paths
      : config.enableMetaFolder ? (_path || _load_path()).default.join(config.cwd, (_constants || _load_constants()).META_FOLDER, 'yarn-error.log') : (_path || _load_path()).default.join(config.cwd, 'yarn-error.log');
    ...
    yield config.init((0, (_extends2 || _load_extends()).default)({
      ...
      errorLogFile: (_commander || _load_commander()).default.errorLogFile,
      ...
      ignoreCpu: (_commander || _load_commander()).default.ignoreCpu,
      ...
      verboseScripts: (_commander || _load_commander()).default.verboseScripts
    })).then(function () {
